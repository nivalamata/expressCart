...
exports.reduceProductQuantity = (req, res) => {
    let db = req.app.db;


    _(req.session.cart).forEach((item) => {
        let productId = exports.getId(item.productId)
        ...
        db.products.findOne({
            ...
            if (err) {
                console.error(colors.red('Error getting product' + productId, err));
            }
            //restrict negative quantity
            let delta = product.productQuantity - quantity;
            db.products.update({
                _id: exports.getId(product._id)
            }, {
                $set: {
                    productQuantity:( delta > 0 ? delta : 0)
                }
            }, {
                multi: false
            }, (err, numReplaced) => {
                if (err) {
                    console.error(colors.red('Failed to update the product quantity: ' + err));
                } else {
                    console.log(colors.cyan('product quantity updated'));
                }
            });
